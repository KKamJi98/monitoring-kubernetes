...
def watch_event_monitoring():
    ...
    if event_choice == "2":
        cmd = f'kubectl get events -A --field-selector type!=Normal --sort-by=".metadata.managedFields[].time" | tail -n {tail_num}'
    else:
        cmd = f'kubectl get events -A --sort-by=".metadata.managedFields[].time" | tail -n {tail_num}'
    print(f"\n실행 명령어: {cmd}\n(Ctrl+C로 중지)\n")
    try:
        while True:
            os.system("clear")
...
def watch_pod_monitoring_by_creation():
    ...
    if extra.startswith("y"):
        cmd = f'kubectl get po -A -o wide --sort-by=.metadata.creationTimestamp | tail -n {tail_num}'
    else:
        cmd = f'kubectl get po -A --sort-by=.metadata.creationTimestamp | tail -n {tail_num}'
    print(f"\n실행 명령어: {cmd}\n(Ctrl+C로 중지)\n")
    try:
        while True:
            os.system("clear")
        ...
        print("\nStopped watching pods by creation.")
...
def watch_non_running_pod():
    ...
    tail_num = get_tail_lines("몇 줄씩 확인할까요? (예: 30): ")
    if extra.startswith("y"):
        cmd = f'kubectl get pods -A -o wide | grep -ivE \'Running\' | tail -n {tail_num}'
    else:
        cmd = f'kubectl get pods -A | grep -ivE \'Running\' | tail -n {tail_num}'
    print(f"\n실행 명령어: {cmd}\n(Ctrl+C로 중지)\n")
    try:
        while True:
            os.system("clear")
        ...
        print("\nStopped watching non-running pods.")
...
def watch_node_monitoring_by_creation():
    ...
    if filter_nodegroup:
        cmd = f'{cmd_base} | grep {filter_nodegroup} | tail -n {tail_num}'
    else:
        cmd = f'{cmd_base} | tail -n {tail_num}'
    print(f"\n실행 명령어: {cmd}\n(Ctrl+C로 중지)\n")
    try:
        while True:
            os.system("clear")
        ...
        print("\nStopped watching nodes by creation.")
...
def watch_unhealthy_nodes():
    ...
    cmd_base = "kubectl get nodes -L topology.ebs.csi.aws.com/zone -L node.kubernetes.io/instancegroup --sort-by=.metadata.creationTimestamp"
    if filter_nodegroup:
        cmd = f'{cmd_base} | grep -ivE \' Ready\' | grep {filter_nodegroup} | tail -n {tail_num}'
    else:
        cmd = f'{cmd_base} | grep -ivE \' Ready\' | tail -n {tail_num}'
    print(f"\n실행 명령어: {cmd}\n(Ctrl+C로 중지)\n")
    try:
        while True:
            os.system("clear")
        ...
        print("\nStopped watching unhealthy nodes.")
...
def watch_node_resources():
    ...
    try:
        ...
        print("\nStopped watching node resources.")
